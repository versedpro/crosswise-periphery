...
import '../interfaces/IFoxchainCallee.sol';
...
contract ExampleFlashSwap is IFoxchainCallee {
    ...
    function foxchainCall(address sender, uint amount0, uint amount1, bytes calldata data) external override {
        address[] memory path = new address[](2);
        uint amountToken;
        uint amountETH;
        { // scope for token{0,1}, avoids stack too deep errors
        address token0 = IFoxchainPair(msg.sender).token0();
        address token1 = IFoxchainPair(msg.sender).token1();
